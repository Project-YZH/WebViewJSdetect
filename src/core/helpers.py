...
def parse_chrome_extension(G, path, dx, easy_test, start_node_id=0):
    ...
    if generated_extension_dir:
        ...
        if result==None:
            ...
            try:
                #print(result)
                G.import_from_string(result)##这里怎么回事？没成功？
            except:
...
def preprocess_cs_bg_war(extension_path, generated_extension_dir, header_path, header, war):
    def processFile(files, newname, relative_path=extension_path):
        ...
        if len(filtered_js_files) > 0 and header:
            filtered_js_files.insert(0, os.path.join(header_path, 'header.js'))
            # filtered_js_files.insert(0, os.path.join(header_path, 'jquery_header.js'))
    ...
    for root, _, files in os.walk(extension_path):
        for file in files:
            if file == 'combined.js':
                continue
            if file.endswith('.js') and 'jquery' not in file.lower() and os.path.getsize(os.path.join(root, file)) != 0:
                js_files.append(os.path.relpath(os.path.join(root, file), extension_path))

    processFile(js_files, 'combined.js')
