...
class OPGen:
    ...
    #Class Definition: OPGen
    def __init__(self):  #初始化
        self.options = options
        # initialize header lines here #头文件路径
        ...
        self.graph = Graph(cs_header_lines=self.cs_header_lines + self.jq_header_lines, \
                           ...
                           thread_version=self.options.run_with_pq)  #创建Graph
        self.graph.package_name = options.input_file
        setup_graph_env(self.graph)  #调用setup_graph_env

    def get_graph(self):  #返回当前的 Graph 对象。
    ...
    def check_vuls(self, vul_type, G):
        ...
        # add chrome extension part
            ...
            if options.obj_traceback:
                ...
                vul_pathes = vul_checking(G, pathes[0], vul_type)
        #使用不同的工具（traceback 和 obj_traceback）来生成路径并检查漏洞
        return vul_pathes
    ...
    def parse_run_extension(self, G, extension_path, dx, res_dir, result_file, vul_type, timeout_s):
        ...
        Error_msg = self._test_graph(G, extension_path, timeout_s, vul_type=vul_type)#重要入口
        if Error_msg != "":
            print(Error_msg)
            with open(os.path.join(res_dir, result_file), 'a') as f:
                f.write(Error_msg + "\n")
    ...
    def _test_graph(self, G: Graph, extension_path, timeout_s, vul_type='os_command'):
        ...
        if timeout_s is not None:
            ...
            internal_plugins = PluginManager(G, init=True)
            entry_id = '0'
            generate_branch_graph(G)#重要分支图
            generate_obj_graph(G, internal_plugins, entry_nodeid=entry_id)#重要对象图???
            if not (G.thread_version and G.ablation_mode in ["coco-single", "coco"]):
                if G.thread_version:
                    G.mydata_to_graph()
                G.thread_version = False
    ...
    def run(self):#主运行方法，根据命令行选项执行：
        ...
        if options.list is not None:
            ...
            elif options.chrome_extension:#这里是入口，走的是这个模式。
...
# the function to admin the threads, to use this, you have to pass G and the initial running thread  线程管理重点关注！！
