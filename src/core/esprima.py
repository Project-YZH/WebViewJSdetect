...
def esprima_parse(path='-', args=[], input=None, print_func=print):
    ...
    proc = subprocess.Popen(['node', main_js_path, path] + args, text=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def esprima_search(module_name, search_path, print_func=print):
    proc = subprocess.Popen(['node', search_js_path, module_name, search_path], text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
