...
class HandleBinaryOP(Handler):
    ...
    def process(self):
        ...
        if flag == 'BINARY_BOOL_OR':
            ...
            handled_left = handle_node(left_child, extra)
            handled_right = handle_node(right_child, extra) #对应于抽象规则的(𝑥₁, σ) ⇒ (Δ₁, Σ₁, Θ₁) (𝑥₂, σ) ⇒ (Δ₂, Σ₂, Θ₂)
            used_objs = []
            used_objs.extend(handled_left.obj_nodes)
            used_objs.extend(handled_right.obj_nodes)
            used_objs = list(set(used_objs))#把 Δ₁ 和 Δ₂ 合并到 Δ′
            # calculate values
            values1, sources1, tags1 = to_values(G, handled_left, node_id)
            values2, sources2, tags2 = to_values(G, handled_right, node_id) #提取Σ₁、Θ₁ 和 Σ₂、Θ₂
            results = []
            result_sources = []
            result_tags = []
            for i, v1 in enumerate(values1):
                for j, v2 in enumerate(values2):
                    ...
                    result_sources.append((sources1[i] or []) + (sources2[j] or [])) #生成𝑥𝑛𝑒𝑤
            ...
            return NodeHandleResult(values=results, used_objs=used_objs,
                value_sources=result_sources, callback=get_df_callback(G)) #对应action部分values 存放 Δ′ 上 xₙₑ𝑤 的值；used_objs 即新的 Δ′ 中与 xₙₑ𝑤 相关的对象节点；value_sources 和 tags 就是 Σ′ 与 Θ′；callback 负责把这条“流”写入底层图结构 Δ。
